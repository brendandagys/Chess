...
```bash
chess$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
chess$ sam local start-api
chess$ curl http://localhost:3000/
```
...
```bash
chess$ sam logs -n HelloWorldFunction --stack-name chess --tail
```
